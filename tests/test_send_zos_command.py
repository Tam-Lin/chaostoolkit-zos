...
from chaoszos.__send_zos_command import EmptyCommand, Send_Command
...
def test_send_command_using_ssh_command_succeeds_v1_1_1(mock_paramiko_client):
    ...
    mock_paramiko_client.return_value.exec_command.return_value = (
        stdin,
        stdout,
        stderr,
    )
    ...
    command_output = Send_Command(
        location, connection_information, "D M=CORE", "IEE174I"
    )

    assert command_output.message_out[0] == "IEE174I 12.41.20 DISPLAY M 813"
    assert (
        command_output.message_out[-1]
        == "MIF ID   MULTIPLE IMAGE FACILITY IMAGE IDENTIFIER"
    )
...
def test_send_command_using_ssh_command_succeeds_v1_1_1_b(mock_paramiko_client):
    ...
    stdout = [
        "CB8A      202216   16:27:20.64             ISF031I CONSOLE MCKENZI ACTIVATED",
